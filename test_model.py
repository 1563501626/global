...
def read_data():
    ...
    return image_batch, label_batch, y_true
...
def main():
    ...
    """
    # 读取数据
    image_batch, label_batch, y_true = read_data()
    ...
    with tf.Session() as sess:
        saver.restore(sess, './models/captcha01/first')
        ...
        for i in range(10):
            print("预测值为:%s, 真实值为:%s" % (
                sess.run(tf.argmax(tf.reshape(y_predict, [BATCH_SIZE, 5, 14]), axis=2)),
                sess.run(y_true)
            ))
